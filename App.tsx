    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 selection:bg-blue-500/30 font-sans overflow-x-hidden">
      <header className="h-20 bg-slate-950/90 backdrop-blur-xl border-b border-slate-900 flex items-center justify-between px-10 sticky top-0 z-50 shadow-2xl">
        ...
        <nav className="flex items-center gap-2 bg-slate-900/40 p-1.5 rounded-2xl border border-slate-800/50">
           <button onClick={() => setActiveTab(NavigationTab.Upload)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === NavigationTab.Upload ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>
              <i className="fa-solid fa-microchip"></i> Neural Scan
           </button>
           <button onClick={() => setActiveTab(NavigationTab.CriminalID)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === NavigationTab.CriminalID ? 'bg-rose-600 text-white' : 'text-slate-500'}`}>
              <i className="fa-solid fa-magnifying-glass"></i> Search Hub
           </button>
           <button onClick={() => setActiveTab(NavigationTab.TacticalMap)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === NavigationTab.TacticalMap ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>
              <i className="fa-solid fa-map-location-dot"></i> Tactical Map
           </button>
           <button onClick={() => setActiveTab(NavigationTab.Archive)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === NavigationTab.Archive ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center gap-2 text-[10px] font-black uppercase tracking-widest mr-4">
             ...
             <span className={isSimulated ? 'text-amber-500' : 'text-emerald-500'}>{isSimulated ? 'SIM' : 'LIVE'}</span>
          </div>
      ...
      <footer className="py-8 text-center opacity-30 text-[10px] font-black text-slate-500 uppercase tracking-[0.8em]">
        Sentinel v4.0.2 // Autonomous GIS Engine
      </footer>
    </div>
  );
};
